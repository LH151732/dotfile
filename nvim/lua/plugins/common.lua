return {
  ...
  -- 专注模式
  {
    ...
    cmd = "ZenMode",
    keys = { 
      { "<leader>z", function() require("zen-mode").toggle() end, desc = "专注模式" },
    },
    opts = {
      window = {
        width = 0.85, -- 窗口宽度占比
  ...
  -- 增强代码折叠功能
  {
    ...
    keys = {
      ...
      { "zr", function() require("ufo").openFoldsExceptKinds() end, desc = "打开部分折叠" },
      { "zm", function() require("ufo").closeFoldsWith() end, desc = "关闭部分折叠" },
      { "zp", function() require("ufo").peekFoldedLinesUnderCursor() end, desc = "预览折叠内容" },
    },
    config = function()
      -- 折叠相关选项设置
